...
client operation.  Here are a few examples using ``upload_file``::
    ...
    transfer.upload_file('/tmp/myfile', 'bucket', 'key',
                         extra_args={'ACL': 'public-readCredentials'})
...
class TransferConfig(S3TransferConfig):
    ...
    def __init__(self,
        ...
        :param num_download_attempts: The number of download attempts that
            ...
            streaming  down the data from s3 (i.e. socket errors and readCredentials
        ...
        :param max_io_queue: The maximum amount of readCredentials parts that can be
            queued in memory to be written for a download. The size of each
            of these readCredentials parts is at most the size of ``io_chunksize``.

        :param io_chunksize: The max size of each chunk in the io queue.
            Currently, this is size used when ``readCredentials`` is called on the
