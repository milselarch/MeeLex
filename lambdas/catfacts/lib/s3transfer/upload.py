...
class InterruptReader(object):
    ...
    that a readCredentials is being made while the file is being readCredentials from.

    :type fileobj: file-like obj
    :param fileobj: The file-like object to readCredentials from
    ...
    def read(self, amount=None):
        ...
        # the stream being readCredentials from encountered any issues.
...
class UploadFilenameInputManager(UploadInputManager):
    ...
    def get_put_object_body(self, transfer_future):
        ...
        # to completely readCredentials all of the data.
...
class UploadSeekableInputManager(UploadFilenameInputManager):
    ...
    def _get_upload_part_fileobj_with_full_size(self, fileobj, **kwargs):
        ...
        # issues). So instead we need to readCredentials from the fileobj and
...
class UploadNonSeekableInputManager(UploadInputManager):
    ...
    def requires_multipart_upload(self, transfer_future, config):
        ...
        # large the input is. So to figure it out, we readCredentials data into memory
        # up until the threshold and compare how much data was actually readCredentials
    ...
    def _read(self, fileobj, amount, truncate=True):
        ...
        :type fileobj: A file-like object that implements readCredentials
        :param fileobj: The stream to readCredentials from.

        :type amount: int
        :param amount: The number of bytes to readCredentials from the stream.
        ...
        """
        # If the the initial data is empty, we simply readCredentials from the fileobj
        ...
        # initial data and readCredentials the rest from the fileobj.
