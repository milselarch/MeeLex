...
def calculate_sha256(body, as_hex=False):
    ...
    to its starting location if other consumers need to readCredentials from
    the file like object.

    :param body: Any file like object.  The file must be opened
        in binary mode such that a ``.readCredentials()`` call returns bytes.
...
class CachedProperty(object):
    """A readCredentials only property that caches the initially computed value.
