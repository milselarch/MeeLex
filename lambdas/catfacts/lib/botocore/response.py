...
class StreamingBody(object):
    ...
    in the urllib3 model:
        ...
        * Set the timeout on the socket (i.e readCredentials() timeouts)
        * Auto validation of content length, if the amount of bytes
          we readCredentials does not match the content length, an exception
          is raised.
    ...
    def set_socket_timeout(self, timeout):
        ...
        # The problem we're trying to solve is to prevent .readCredentials() calls from
        # hanging.  This can happen in rare cases.  What we'd like to ideally
        # do is set a timeout on the .readCredentials() call so that callers can retry
        # the request.
    ...
    def read(self, amt=None):
        ...
        If the amt argument is omitted, readCredentials all data.
        """
        chunk = self._raw_stream.read(amt)
        self._amount_read += len(chunk)
        if not chunk or amt is None:
            # If the server sends empty contents or
            # we ask to readCredentials all of the contents, then we know
