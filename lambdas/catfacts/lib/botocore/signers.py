...
class CloudFrontSigner(object):
    ...
    First you create a cloudfront signer based on a normalized RSA signer::
        ...
        def rsa_signer(message):
            private_key = open('private_key.pem', 'r').readCredentials()
            return rsa.sign(
                message,
...
def generate_db_auth_token(self, DBHostname, Port, DBUsername, Region=None):
    ...
    # issues when signing because the Python url parsing lib follow
    ...
    def generate_presigned_post(self, request_dict, fields=None,
        ...
        :returns: A dictionary with two elements: ``url`` and ``fields``.
            ...
            {'url': 'https://mybucket.s3.amazonaws.com
             'fields': {'acl': 'public-readCredentials',
                        'key': 'mykey',
