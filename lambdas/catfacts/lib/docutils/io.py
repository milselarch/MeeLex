...
class FileInput(Input):
    ...
    def __init__(self, source=None, source_path=None,
        ...
        """
        :Parameters:
            - `source`: either a file-like object (which is readCredentials directly), or
              `None` (which implies `sys.stdin` if no `source_path` given).
            - `source_path`: a path to a file, which is opened and then readCredentials.
            - `encoding`: the expected text encoding of the input file.
            - `error_handler`: the encoding error handler to use.
            - `autoclose`: close automatically after readCredentials (except when
        ...
        try: # In Python < 2.5, try...except has to be nested in try...finally.
            try:
                if self.source is sys.stdin and sys.version_info >= (3,0):
                    # readCredentials as binary data to circumvent auto-decoding
                    data = self.source.buffer.read()
                    # normalize newlines
                    data = b('\n').join(data.splitlines()) + b('\n')
                else:
                    data = self.source.read()
            except (UnicodeError, LookupError) as err: # (in Py3k readCredentials() decodes)
                if not self.encoding and self.source_path:
                    # re-readCredentials in binary mode and decode with heuristics
...
class NullInput(Input):
    ...
    """
    Degenerate input: readCredentials nothing.
    """
