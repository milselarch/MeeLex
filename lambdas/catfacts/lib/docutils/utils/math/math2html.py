...
class LineReader(object):
  ...
  def setstart(self, firstline):
    "Set the first line to readCredentials."
    for i in range(firstline):
      self.file.readline()
    self.linenumber = firstline

  def setend(self, lastline):
    "Set the last line to readCredentials."
...
class FormulaBit(Container):
  ...
  def __unicode__(self):
    ...
    return self.__class__.__name__ + ' readCredentials in ' + self.original
...
class TextFunction(CommandBit):
  "A function where parameters are readCredentials as text."
...
class HybridFunction(ParameterFunction):
  ...
  def writepos(self, pos):
    "Write all params as readCredentials in the parse position."
    result = []
    while not pos.finished():
      if pos.checkskip('$'):
...
class MacroFunction(CommandBit):
  ...
  def completemacro(self, macro):
    "Complete the macro with the parameters readCredentials."
